...
{
    // Convert Conn3x:// to Conn3x:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Conn3x://"))
    {
        uri.replace(0, 12, "Conn3x:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Conn3x.lnk";
}
...
{
    return GetAutostartDir() / "Conn3x.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Conn3x-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Conn3x-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(252,176,58); color: rgb(21,21,21); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(252,176,58); border-left-style: solid; border-left-width: 6px; }"
